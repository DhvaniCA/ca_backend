...
# # ── Added: optional image_data field for vision support ──────────────────────
# class ChatRequest(BaseModel):
#     message:    str
#     history:    Optional[List[ChatMessage]] = None
#     mode:       Optional[str] = "qa"    # "qa" | "discussion"
...
# async def is_ca_related_question(question: str, is_from_image: bool = False) -> bool:
#     """
#     Domain classifier for the CA gatekeeper.

#     is_from_image=True means the text was already produced by GPT-4o vision
#     from a photo/screenshot the student explicitly uploaded as study material —
#     in that case we trust it and skip the strict LLM classifier entirely, since
#     running the classifier on OCR'd text is what was causing valid CA content
#     to get rejected too often.
#     """
#     # Fast-path: always allow questions about Dhvani itself
#     _dhvani_keywords = [
#         "dhvani", "founder", "promoter", "abhinav", "monali", "uma aggarwal",
...
#     # Image-sourced content is already visually confirmed to be study material —
#     # trust it and skip the strict classifier.
#     if is_from_image:
#         return True

#     system = (
#         "You are a lenient domain classifier for an Indian Chartered Accountancy (CA) "
#         "student assistant.\n\n"
#         "Answer YES if the question relates even loosely to:\n"
#         "- ICAI syllabus, Accounting, Auditing, Direct Tax/GST, Corporate Law,\n"
#         "  Financial management, Costing, Economics, or any commerce/business/finance topic.\n"
#         "- General education, exam preparation, or study-skills questions.\n"
#         "- The Dhvani app, its founders, team, features, or anything about this platform.\n\n"
#         "Answer NO only if the question is clearly and entirely unrelated to commerce/education "
#         "(e.g. pure entertainment, sports scores, unrelated coding help, celebrity gossip).\n"
...
#         # ── Step 0b: Vision — if image_data present, extract text via OCR ────
#         # ocr_text holds the CLEAN extracted text (no labels/prefixes). This is
#         # what drives the CA gatekeeper + RAG retrieval below, so retrieval
#         # quality isn't hurt by wrapper text like "[Text extracted from image]:".
#         ocr_text = ""
#         if req.image_data:
#             try:
#                 raw = req.image_data
...
#                                 "type": "text",
#                                 "text": (
#                                     "Look at this image from an Indian CA (Chartered Accountancy) "
#                                     "student's study material, notes, or question paper. "
#                                     "Extract and clearly restate the underlying topic, question, "
#                                     "or problem in well-structured plain text — a short heading "
#                                     "line naming the topic/subject, followed by the full question "
#                                     "or content, exactly as it appears. Don't worry about being "
#                                     "conservative: if it looks even loosely related to accounting, "
#                                     "tax, audit, law, costing, finance, or commerce/education "
#                                     "generally, extract it fully. Return only the extracted/"
#                                     "restated text, no commentary."
#                                 ),
#                             },
#                             {
#                                 "type": "image_url",
...
#                 if ocr_text:
#                     # Keep req.message readable for history/logging, but don't
#                     # wrap it in labels that would hurt embedding quality.
#                     if req.message and req.message not in (
#                         "(File uploaded — see attachment)",
#                         "Please read the question or content in this image and answer it from a CA exam perspective.",
#                     ):
...
#         # Clean text used for classification + retrieval (prefers OCR output)
#         rag_query_text = ocr_text if ocr_text else req.message

#         # --------------------------------------------------
#         # Step 1: CA gatekeeper
#         # --------------------------------------------------
#         if not await is_ca_related_question(rag_query_text, is_from_image=bool(req.image_data)):
#             return ChatResponse(
#                 answer=(
#                     "⚠️ This doesn't appear to be a CA or business-related question.\n\n"
#                     "This assistant is designed for Indian CA students. "
...
#         # Step 3: RAG — Pinecone vector search
#         # --------------------------------------------------
#         query_embedding = await embed_single(enrich_query_for_rag(rag_query_text))
#         detected_subj   = detect_subject(rag_query_text)
...
#         # Sort by score + dynamic threshold
#         matches = sorted(matches, key=lambda m: m.get("score", 0), reverse=True)
#         q_len   = len(rag_query_text.split())
#         thr     = 0.52 if q_len <= 6 else 0.60 if q_len <= 15 else 0.65
#         # OCR'd text rarely matches chunk wording as tightly as typed questions —
#         # loosen the bar for image-based queries so RAG context still surfaces.
#         if req.image_data:
#             thr = max(thr - 0.10, 0.40)
#         matches = [m for m in matches if m.get("score", 0) >= thr] or matches[:5]
...
# ── Video processing self-healing constants ───────────────────────────────────
# Real video generation jobs take roughly 30-45 minutes end-to-end. We give a
# buffer above that ceiling so we don't falsely kill a healthy in-flight job,
# but still guarantee that a crashed/abandoned job (e.g. the external video
# worker died and never wrote back a final status) gets automatically
# reclassified as "failed" instead of showing "processing" forever.
MAX_VIDEO_PROCESSING_MINUTES = getattr(settings, "MAX_VIDEO_PROCESSING_MINUTES", 50)
...
async def validate_config():
    ...
    print(f"[startup] EMBEDDING_MODEL = {settings.EMBEDDING_MODEL}")
    print(f"[startup] Gemini summary  = {'enabled' if gemini_key else 'disabled (set GEMINI_API_KEY to enable)'}")
    print(f"[startup] MAX_VIDEO_PROCESSING_MINUTES = {MAX_VIDEO_PROCESSING_MINUTES}")
...
# ============================================================
# VIDEO PROCESSING — SELF-HEALING STATUS HELPERS
# ============================================================
#
# Problem this section fixes:
#   The item's `processing_status` gets set to "processing" right when a
#   video job starts. Video generation genuinely takes ~30-45 minutes, but
#   if the external video worker crashes, times out, or never calls back
#   with a final "completed"/"failed" status, the DB is left holding
#   "processing" forever — the dashboard would show a spinner indefinitely
#   with no way for the student or admin to retry.
#
# Fix:
#   - Every time we start a job we now stamp `processing_started_at`.
#   - Any read of an item (tree or single item) computes an *effective*
#     status: if it's been "processing" for longer than
#     MAX_VIDEO_PROCESSING_MINUTES and there's still no video_url, we treat
#     it as "failed" so the UI shows a retry button — without needing the
#     crashed worker to ever come back.
#   - We also persist that correction back to Mongo the first time we
#     notice it, so every future reader sees the corrected state too.
# ============================================================
...
def _effective_processing_status(doc: dict) -> Optional[str]:
    """Self-heals a stuck 'processing' status without needing the external
    video worker to ever write back a final status."""
    status = doc.get("processing_status")

    if status != "processing":
        return status

    # If a video_url already exists, the job clearly finished — treat as completed
    # even if the final status write got lost somehow.
    if doc.get("video_url"):
        return "completed"

    started_at = _parse_dt(doc.get("processing_started_at"))
    if not started_at:
        # No reliable start time to judge against — don't leave it spinning forever.
        return "failed"

    if datetime.utcnow() - started_at > timedelta(minutes=MAX_VIDEO_PROCESSING_MINUTES):
        return "failed"

    return "processing"


async def _self_heal_status(doc_id_str: str, doc: dict) -> str:
    """Computes the effective status and persists the correction if the
    stored status was stale, so every client sees the corrected state."""
    effective = _effective_processing_status(doc)
    if effective == "failed" and doc.get("processing_status") == "processing":
        try:
            await dashboard_collection.update_one(
                {"_id": ObjectId(doc_id_str)},
                {"$set": {"processing_status": "failed"}},
            )
            print(f"[self-heal] {doc_id_str}: stale 'processing' -> 'failed'")
        except Exception as e:
            print(f"[self-heal] Could not persist correction for {doc_id_str}: {e}")
    return effective
...
# ============================================================
#
# CHAINING FIX (see explanation below):
#   1. save_turn_and_maybe_summarize() now uses an ATOMIC Mongo update
#      ($push with $slice + $inc) instead of a read-modify-write $set.
#      This removes the race condition where two closely-spaced /chat
#      calls could overwrite each other's saved turns.
#   2. It no longer needs a stale `current_memory` snapshot passed in —
#      it fetches the post-update document straight from the atomic op,
#      so it always summarizes from the true, current state.
#   3. /chat now schedules this via FastAPI's BackgroundTasks instead of
#      asyncio.create_task(), so the task is guaranteed to run to
#      completion and can't be silently garbage-collected mid-flight.
# ============================================================
...
async def save_turn_and_maybe_summarize(
    ...
    """
    Atomically appends the new Q+A turn and trims the sliding window in a
    single Mongo operation ($push + $slice + $inc), instead of the old
    read-then-$set pattern. This means two /chat calls that finish close
    together can no longer stomp on each other's saved turns — each
    request only ever appends, it never overwrites the whole array from a
        ...
        result = await users_collection.find_one_and_update(
            {"email": user_email},
            {
                "$push": {
                    "conversation_turns": {
                        "$each":  new_entries,
                        "$slice": -(MAX_TURNS * 2),   # keep only last MAX_TURNS Q+A pairs
                    }
                },
                "$inc": {"total_turns": 1},
            },
            return_document=True,
            projection={"conversation_turns": 1, "conversation_summary": 1, "total_turns": 1},
        )
    except Exception as e:
        print(f"[memory] Failed to save turn for {user_email}: {e}")
        return
    ...
    if not result:
        print(f"[memory] Failed to save turn for {user_email}: user not found")
        return

    new_total = result.get("total_turns", 0)

    if new_total and new_total % SUMMARIZE_EVERY == 0:
        turns = result.get("conversation_turns", [])
        turns_text = "\n".join(
            f"{t['role'].upper()}: {t['content'][:400]}"
            for t in turns
        )
        existing_summary = result.get("conversation_summary", "")
        ...
        if new_summary:
            try:
                ...
                print(f"[memory] Failed to save summary for {user_email}: {e}")
...
async def get_dashboard_tree(user=Depends(get_current_user)):
    ...
    Also self-heals any item that's been stuck in 'processing' for longer
    than MAX_VIDEO_PROCESSING_MINUTES (e.g. the external video worker
    crashed) — those get reported as 'failed' so the frontend shows a
    retry button instead of an endless spinner.
    """
    tree: Dict[str, Any] = {}
    async for doc in dashboard_collection.find().sort("created_at", 1):
        doc_id = str(doc["_id"])
        ...
        effective_status = await _self_heal_status(doc_id, doc)
...
# ============================================================
# VIDEO PROCESSING — LIFECYCLE ENDPOINTS
# ============================================================
#
# These two endpoints let the frontend record job lifecycle state directly
# in MongoDB *before/around* calling the (separate, less reliable) video
# worker service. That way, even if the worker process crashes outright and
# never calls back, we always have:
#   - a "processing" status
#   - a processing_started_at timestamp
# ...which is everything _effective_processing_status() needs to self-heal
# the item back to "failed" after MAX_VIDEO_PROCESSING_MINUTES, without
# requiring a human to go edit MongoDB by hand.
# ============================================================

@app.post("/dashboard/item/{item_id}/mark-processing")
async def mark_item_processing(item_id: str, admin=Depends(get_current_admin)):
    try:
...
@app.post("/dashboard/item/{item_id}/mark-failed")
async def mark_item_failed(item_id: str, admin=Depends(get_current_admin)):
    try:
        obj_id = ObjectId(item_id)
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid item id")

    result = await dashboard_collection.update_one(
        {"_id": obj_id},
        {"$set": {"processing_status": "failed"}},
    )
    if result.matched_count == 0:
    ...
    return {"item_id": item_id, "processing_status": "failed", "message": "Marked as failed"}
...
async def chat(req: ChatRequest, background_tasks: BackgroundTasks, user=Depends(get_current_user)):
    try:
        ...
        # --------------------------------------------------
        # Step 2: Load conversation memory (read for prompt-building only —
        # the SAVE side no longer depends on this snapshot; see the atomic
        # save_turn_and_maybe_summarize() below)
        # --------------------------------------------------
        memory       = await get_user_memory(user["email"])
        memory_block = build_memory_block(memory)
        ...
        if not matches:
            ...
            # CHAINING FIX: use BackgroundTasks instead of asyncio.create_task
            # so this save is guaranteed to run and can't be silently GC'd.
            background_tasks.add_task(
                save_turn_and_maybe_summarize, user["email"], req.message, answer
            )
...
@app.get("/dashboard/item/{item_id}")
async def get_dashboard_item(item_id: str, user=Depends(get_current_user)):
    """Also self-heals a stale 'processing' status (see helpers above)."""
    doc = await dashboard_collection.find_one({"_id": ObjectId(item_id)})
    if not doc:
        raise HTTPException(status_code=404, detail="Item not found")

    effective_status = await _self_heal_status(item_id, doc)

    doc["_id"] = item_id
    doc["processing_status"] = effective_status
